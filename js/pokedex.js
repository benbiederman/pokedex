class Pokemon {
    constructor(name, picture, description, element, weakness, evolvesInto, evolutionType, evolutionLevel, number) {
        this.name = name;
...
const charmeleon = new Pokemon('charmeleon', `img/charmeleon.jpg`, `Charmeleon mercilessly destroys its foes using its sharp claws. If it encounters a strong foe, it turns aggressive. In this excited state, the flame at the tip of its tail flares with a bluish white color.`, 'Fire', 'Ground/Rock/Water', 'charizard', 'Normal', 36, 05);

const charizard = new Pokemon('charizard', `img/charizard.jpg`, `Charizard flies around the sky in search of powerful opponents. It breathes fire of such great heat that it melts anything. However, it never turns its fiery breath on any opponent weaker than itself.`, 'Fire', 'Ground/Rock/Water', false, 'Normal', false, 06);

const squirtle = new Pokemon('squirtle', `img/squirtle.jpg`, `Squirtle's shell is not merely used for protection. The shell's rounded shape and the grooves on its surface help minimize resistance in water, enabling this Pokémon to swim at high speeds.`, 'Water', 'Grass/Electric', `wartortle`, 'Normal', 16, 07);

const wartortle = new Pokemon('wartortle', `img/wartortle.jpg`, `Its tail is large and covered with a rich, thick fur. The tail becomes increasingly deeper in color as Wartortle ages. The scratches on its shell are evidence of this Pokémon's toughness as a battler.`, 'Water', 'Grass/Electric', `blastoise`, 'Normal', 36, 08);
...
const butterfree = new Pokemon('butterfree', `img/butterfree.jpg`, `Butterfree has a superior ability to search for delicious honey from flowers. It can even search out, extract, and carry honey from flowers that are blooming over six miles from its nest.`, 'Grass', 'Rock/Fire/Flying/Ice/Electric', false, 'Normal', false, 12);

const weedle = new Pokemon('weedle', `img/weedle.jpg`, `Weedle has an extremely acute sense of smell. It is capable of distinguishing its favorite kinds of leaves from those it dislikes just by sniffing with its big red proboscis (nose).`, 'Grass', 'Fire/Flying/Rock/Psychic', 'kakuna', 'Normal', 7, 13);

const kakuna = new Pokemon('kakuna', `img/kakuna.jpg`, `Kakuna remains virtually immobile as it clings to a tree. However, on the inside, it is extremely busy as it prepares for its coming evolution. This is evident from how hot the shell becomes to the touch.`, 'Grass', 'Fire/Flying/Rock/Psychic', 'beedrill', 'Normal', 10, 14);
...
function pokemonFound(pokemon){
    ...
    switch(pokemon.element) {
        case 'Electric':
            container.style.backgroundColor = '#f6e184';
            break;
        case 'Psychic':
            container.style.backgroundColor = '#ba8aaf';
            break;
        case 'Fire':
            container.style.backgroundColor = '#eb7d75';
            break;
        case 'Ground':
            container.style.background = '#8b7057';
            break;
        case 'Water':
            container.style.backgroundColor = '#7eb1d9';
            break;
        case 'Normal':
            container.style.backgroundColor = '#d5cbc1';
            break;
        case 'Grass':
            container.style.backgroundColor = '#94a568';
            break;
        default:
            container.style.backgroundColor = '#D8D8CC';
            break;
    }
    ...
    pokemonImage.src = pokemon.picture;
    ...
    pokemonDetails.appendChild(pokemonDescription);
    ...
    evolutionDiv.classList.add('info');
    pokemonInfo.appendChild(evolutionDiv);

    const evolution = document.createElement('p');
    evolution.textContent = 'Evolution:';
    evolutionDiv.appendChild(evolution);

    const evolutionName = document.createElement('h4');
    if(pokemon.evolvesInto === false) {
        evolutionName.textContent = 'None';
    } else {
    evolutionName.textContent = pokemon.evolvesInto;
    }
    ...
    pokemonInfo.appendChild(evolutionTypeDiv);

    const evolutionTypeHeader = document.createElement('p');
    evolutionTypeHeader.textContent = 'Evolution Type:';
    evolutionTypeDiv.appendChild(evolutionTypeHeader);

    const evolutionType = document.createElement('h4');
    ...
    pokemonInfo.appendChild(evolutionLevelDiv);

    const evolutionLevelHeader = document.createElement('p');
    evolutionLevelHeader.textContent = 'Evolution Level:';
    evolutionLevelDiv.appendChild(evolutionLevelHeader);

    const evolutionLevel = document.createElement('h4');
    if(pokemon.evolutionLevel === false){
        evolutionLevel.textContent = 'N/A';
    } else {
    ...
    container.appendChild(pokemonNumber);
